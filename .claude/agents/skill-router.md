...
- **explore-beyond-the-task**: open-ended audit prompts — "review this repo", "find what's wrong", "audit", "investigate", "something's broken", "look around", "check the recent feature". The user is NOT pointing at a known bug; they want broad exploration. **For open-ended audits the hunter agents (below) take over the heavy lifting** — they're parallel, read-only, and the PostToolUse hook auto-consolidates their findings into a single file the main worker Reads once. Do NOT route on narrow "fix this specific thing" prompts — that's test-first / bug-fixer territory.
- **multi-hunter (5 parallel read-only Agent dispatches)**: PRIMARY recommendation for open-ended audit prompts. Each hunter applies ONE lens of inquiry to the WHOLE repo, returns structured JSON findings, and never edits. The five lenses:
  - `cross-reference-hunter` — same fact in 2+ places, find disagreements (magic numbers, ports, URLs).
  - `invariant-hunter` — JSDoc/README/ADR assertions ("always/must/never/sorted/leftmost") vs code below.
  - `error-handling-hunter` — catch blocks, fallbacks, `.catch(() => …)` that swallow failures.
  - `boundary-hunter` — API-route handlers reading untrusted input without validation.
  - `surface-hunter` — curl every linked route, parse HTML, flag visible breakage.
  Emit one `prompt-suggested-<hunter>` sentinel per hunter. Reply tells the worker the three-stage flow the hooks enforce: (1) dispatch all 5 hunters — PreToolUse blocks every non-Agent tool until all fire; (2) PostToolUse auto-consolidates their JSON output into `runtime/.harness-state/hunter-findings.md`; (3) PreToolUse blocks Edit/Write/Grep/Glob/exploration-Bash until the worker Reads that consolidated file, then transitions into restricted post-hunter mode where Grep/Glob and `grep -r`/`find`/`rg`/`curl`/`ls -R` are denied — Read the cited files and Edit; that's the path.
...
Sentinels: prompt-suggested-cross-reference-hunter, prompt-suggested-invariant-hunter, prompt-suggested-error-handling-hunter, prompt-suggested-boundary-hunter, prompt-suggested-surface-hunter
Reply: "Recommended: dispatch all 5 hunter agents (cross-reference-hunter, invariant-hunter, error-handling-hunter, boundary-hunter, surface-hunter) — Agent(subagent_type='<name>') × 5, in one message if possible. They run read-only in parallel and each returns structured JSON findings. The harness enforces a strict three-stage flow: (1) PreToolUse denies every non-Agent tool until all 5 hunters fire; (2) PostToolUse auto-consolidates their JSON into runtime/.harness-state/hunter-findings.md; (3) PreToolUse demands you Read that consolidated file before any Edit/Write/Grep/Glob/exploration-Bash, then transitions into restricted post-hunter mode where Grep/Glob and `grep -r`/`find`/`rg`/`curl`/`ls -R` are denied — you Read the cited files and Edit; that's the path. Do NOT plan to re-explore with grep/find after hunters return — the hunters already enumerated, the gate enforces this, and ignoring it burns budget you need for fixes."
