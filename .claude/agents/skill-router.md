...
- **plain-closeout**: explicit user request for a SUMMARY of work done — phrases like "summarise what you did", "give me a writeup of what changed", "wrap this up with a status report", "what's done so far that I can paste to the team", "report back on the refactor". Routes regardless of whether the prior work was code, docs, or research; the trigger is SUMMARISATION INTENT, not the underlying task type. **NOT for quick status inquiries** — "where are we", "what's the current state", "any progress" by themselves are status questions, not summary requests. plain-closeout fires automatically on worker-initiated closeouts via its description; the router only surfaces it when the user explicitly asks for a SUMMARY of completed work.
