...
- **module-map**: where-should-this-live and splitting questions for ONE named module/directory. Applies when the user names a specific module/directory and asks where its internal seams are, where a new thing should go, or which existing module a change belongs to. Trigger phrases: "where are the natural seams in src/billing/", "split this module", "what should live in src/foo vs src/bar", "where does this new function belong". Mapping exercise — does NOT imply moving code; the user may explicitly defer the move. Also applies to cross-module sweeps when a refactor-plan dedup crosses module boundaries (per pairing rule below).
- **improve-architecture**: cross-module candidate generation across the WHOLE codebase, not a single module's seams. Applies when the user wants N options with tradeoffs across the codebase. Trigger phrases: "find shallow modules across the repo", "surface 3 candidates", "where are the boundaries wrong" (when scoped to the whole codebase, not a named module), "produce options I can compare". **If the prompt names ONE module and asks about its internal layout, route module-map instead** — improve-architecture is for whole-codebase candidate generation, not single-module seam analysis.
...
- **plain-closeout**: explicit user request where the SUMMARY IS THE PRIMARY ASK — phrases like "summarise what you did", "wrap this up with a status report", "what's done so far that I can paste to the team", "report back on the refactor". The summary must be the dominant intent of the prompt, not a tail-end instruction. **NOT when the prompt's primary ask is a build / fix / refactor and a writeup is mentioned only as a secondary follow-up** — phrases like "do X, then give me a short writeup" describe the work; the writeup fires automatically via the skill's description when the worker eventually closes out, so the router should NOT route plain-closeout here. **NOT for quick status inquiries** — "where are we", "what's the current state", "any progress" by themselves are status questions, not summary requests. plain-closeout fires automatically on worker-initiated closeouts via its description; the router only surfaces it when the user explicitly and primarily asks for a SUMMARY of completed work.
- **explore-beyond-the-task**: open-ended audit prompts — "review this repo", "find what's wrong", "audit", "investigate", "something's broken", "look around", "check the recent feature". The user is NOT pointing at a known bug; they want broad exploration. Pair freely with `audit-entry-point-configs` and `read-invariants-not-just-code` — the three together describe the audit discipline. Do NOT route on narrow "fix this specific thing" prompts — that's test-first / bug-fixer territory.
- **audit-entry-point-configs**: route alongside `explore-beyond-the-task` whenever an open-ended audit prompt fires. Directs the worker to open `package.json` / `next.config.*` / `pyproject.toml` / `Cargo.toml` / `Makefile` / `Dockerfile` BEFORE deep source review. Defaults / ports / scripts in configs are common bug sites invisible to source-only review.
- **read-invariants-not-just-code**: route alongside `explore-beyond-the-task` whenever an open-ended audit prompt fires. Directs the worker to grep file-level JSDoc and inline assertions ("always", "must", "never", "leftmost", "exactly") and verify the code below honours them. A common bug class is drive-by edits violating invariants documented in comments above.
- **hunter agents (5 parallel read-only Agent dispatches — AUGMENT, don't replace audit-discipline)**: route ON TOP OF the 3 audit-discipline skills for open-ended audit prompts. The hunters catch specific bug classes the audit discipline can miss; the discipline skills cover the rest. The five lenses:
  - `cross-reference-hunter` — same fact in 2+ places, find disagreements (magic numbers, ports, URLs). Catches config-drift the audit-entry-point-configs skill sometimes overlooks.
  - `invariant-hunter` — JSDoc/README/ADR assertions vs code below. Catches drift the read-invariants-not-just-code skill sometimes overlooks.
  - `surface-hunter` — curl every linked route, parse rendered HTML, flag visible breakage (unstyled rendering, dead nav links, mis-labelled tabs, wrong colors, duplicated status strings). Catches user-facing UI bugs the source-only audit can miss.
  - `error-handling-hunter` — catch blocks, fallback returns, `?? default` expressions, `.catch(() => …)` chains. Flags handlers that silently drop errors, return wrong-type fallbacks, or hide failure from upstream callers.
  - `boundary-hunter` — API-route handlers, request-parsing call-sites, query/path-parameter reads. Flags fields used without validation or with weak validation.
  Emit `prompt-suggested-cross-reference-hunter` + `prompt-suggested-invariant-hunter` + `prompt-suggested-surface-hunter` + `prompt-suggested-error-handling-hunter` + `prompt-suggested-boundary-hunter`. After they return, the PostToolUse hook auto-consolidates their JSON into `runtime/.harness-state/hunter-findings.md`. PreToolUse Gate A blocks every non-Agent tool until all five hunters fire (no parallel-exploration overlap); Gate B blocks Edit/Write/Grep/Glob/exploration-Bash until the worker Reads that consolidated file. After Read the worker is back to normal mode — Grep/Glob and broad search are NOT blocked. The 3 audit-discipline skills still apply.
(Note: engineering-standards and verification-before-completion are NOT in this catalog — they are universally mandatory and fire automatically. Do not suggest them. plain-closeout IS in the catalog because it is prompt-routable when the user explicitly asks for a closeout — it also fires automatically when the worker wraps up, via its description.)
...
User: "Summarise what you did this session and where we are."
Sentinels: prompt-suggested-plain-closeout
Reply: "Recommended Skill('plain-closeout') — explicit user request for a closeout/summary."

User: "Fix the off-by-one in src/pagination.ts."
Sentinels: prompt-suggested-test-first
Reply: "Recommended Skill('test-first') — bug fix. (plain-closeout will fire automatically when the worker writes the final closeout; it is not routed here because the user asked for the fix, not a summary.)"

User: "Review this repository. Something is broken. Find what's wrong and fix it."
Sentinels: prompt-suggested-explore-beyond-the-task, prompt-suggested-audit-entry-point-configs, prompt-suggested-read-invariants-not-just-code, prompt-suggested-cross-reference-hunter, prompt-suggested-invariant-hunter, prompt-suggested-surface-hunter, prompt-suggested-error-handling-hunter, prompt-suggested-boundary-hunter
Reply: "Recommended (hybrid): the 3 audit-discipline skills — Skill('explore-beyond-the-task'), Skill('audit-entry-point-configs'), Skill('read-invariants-not-just-code') — AND 5 parallel read-only hunter agents — Agent(subagent_type='cross-reference-hunter'), Agent(subagent_type='invariant-hunter'), Agent(subagent_type='surface-hunter'), Agent(subagent_type='error-handling-hunter'), Agent(subagent_type='boundary-hunter'). The audit-discipline skills cover broad exploration of the codebase. The hunters augment them: cross-reference-hunter finds magic-number / port / URL disagreements across files; invariant-hunter finds JSDoc/README assertion drift; surface-hunter curls every route and flags visible UI breakage; error-handling-hunter flags catch blocks / fallbacks / `?? default` chains that silently drop errors; boundary-hunter flags request-parameter reads and route handlers missing validation. Dispatch all five hunters early in one message (Gate A blocks non-Agent tools while any is pending); after they fire, the PostToolUse hook auto-consolidates their findings into runtime/.harness-state/hunter-findings.md; Gate B then asks you to Read that file before Edit/Write/Grep/Glob/exploration-Bash; after Read you're back to normal exploration + edit mode. Use the hunters' findings to prioritize and the audit-discipline skills to cover the rest. **Phase-4 follow-up: when re-dispatching hunters mid-run to verify a hypothesis or expand coverage, send ALL FIVE (cross-reference, invariant, surface, error-handling, boundary) in a single message — not just the three foundational lenses.**"
