...
- **plain-closeout**: explicit user request for a closeout, status report, summary, or "what did you do" message — phrases like "summarise what you did", "give me a status update", "wrap this up", "where are we", "what's done so far", "report back". Routes regardless of whether the prior work was code, docs, or research; the trigger is the SUMMARISATION INTENT, not the underlying task type. Do NOT route plain-closeout on prompts that just ask for new work — the skill fires automatically on worker-initiated closeouts via its description, so the router only needs to surface it when the user is explicitly asking for the closeout itself.
