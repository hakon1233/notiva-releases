...
- **hunter agents (3 parallel read-only Agent dispatches — AUGMENT, don't replace audit-discipline)**: route ON TOP OF the 3 audit-discipline skills for open-ended audit prompts. The hunters catch specific bug classes the audit discipline can miss; the discipline skills cover the rest. The three lenses:
  ...
  Emit `prompt-suggested-cross-reference-hunter` + `prompt-suggested-invariant-hunter` + `prompt-suggested-surface-hunter`. After they return, the PostToolUse hook auto-consolidates their JSON into `runtime/.harness-state/hunter-findings.md`. PreToolUse Gate A blocks every non-Agent tool until all three hunters fire (no parallel-exploration overlap); Gate B blocks Edit/Write/Grep/Glob/exploration-Bash until the worker Reads that consolidated file. After Read the worker is back to normal mode — Grep/Glob and broad search are NOT blocked. The 3 audit-discipline skills still apply.
...
Sentinels: prompt-suggested-explore-beyond-the-task, prompt-suggested-audit-entry-point-configs, prompt-suggested-read-invariants-not-just-code, prompt-suggested-cross-reference-hunter, prompt-suggested-invariant-hunter, prompt-suggested-surface-hunter
Reply: "Recommended (hybrid): the 3 audit-discipline skills — Skill('explore-beyond-the-task'), Skill('audit-entry-point-configs'), Skill('read-invariants-not-just-code') — AND 3 parallel read-only hunter agents — Agent(subagent_type='cross-reference-hunter'), Agent(subagent_type='invariant-hunter'), Agent(subagent_type='surface-hunter'). The audit-discipline skills cover broad exploration of the codebase. The hunters augment them: cross-reference-hunter finds magic-number / port / URL disagreements across files; invariant-hunter finds JSDoc/README assertion drift; surface-hunter curls every route and flags visible UI breakage. Dispatch all three hunters early in one message (Gate A blocks non-Agent tools while any is pending); after they fire, the PostToolUse hook auto-consolidates their findings into runtime/.harness-state/hunter-findings.md; Gate B then asks you to Read that file before Edit/Write/Grep/Glob/exploration-Bash; after Read you're back to normal exploration + edit mode. Use the hunters' findings to prioritize and the audit-discipline skills to cover the rest."
