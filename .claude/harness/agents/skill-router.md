...
Sentinels: prompt-suggested-explore-beyond-the-task, prompt-suggested-audit-entry-point-configs, prompt-suggested-read-invariants-not-just-code
Reply: "Recommended Skill('explore-beyond-the-task'), Skill('audit-entry-point-configs'), and Skill('read-invariants-not-just-code') — open-ended audit prompt with no specific bug pointed at; the three together cover breadth, config-level defects, and JSDoc-invariant violations that source-only review misses."
