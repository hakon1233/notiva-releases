...
- **explore-beyond-the-task**: open-ended audit prompts — "review this repo", "find what's wrong", "audit", "investigate", "something's broken", "look around", "check the recent feature". The user is NOT pointing at a known bug; they want broad exploration. **For open-ended audit prompts, prefer the parallel hunter agents (below) — they replace this skill's narrative discipline with mechanical lens-of-inquiry agents that return structured JSON.** Do NOT route on narrow "fix this specific thing" prompts — that's test-first / bug-fixer territory.
- **multi-hunter (5 parallel read-only Agent dispatches)**: PRIMARY recommendation for open-ended audit prompts. Each hunter applies ONE lens of inquiry to the WHOLE repo, returns structured JSON findings, and never edits. The five lenses:
  ...
  Emit `prompt-suggested-<hunter-name>` × 5. The Reply tells the main worker to dispatch ALL hunters BEFORE any other tool call — the PreToolUse hook blocks Bash/Read/Grep/Glob/Edit/Write until every hunter has fired, so attempting to explore in parallel will be denied.
...
Reply: "Recommended: dispatch all 5 hunter agents — Agent(subagent_type='cross-reference-hunter'), Agent(subagent_type='invariant-hunter'), Agent(subagent_type='error-handling-hunter'), Agent(subagent_type='boundary-hunter'), Agent(subagent_type='surface-hunter'). They run read-only and return structured JSON findings (lens + findings array). IMPORTANT: the harness PreToolUse hook will DENY any Bash/Read/Grep/Glob/Edit/Write/Skill call until every hunter has fired — so dispatch them all (in one message if possible) before attempting any other tool. After all 5 return as tool_results in your next user-turn, consolidate (dedupe by file+line±5, drop low-confidence non-high-severity findings), prioritize by severity+overlap, then fix each in sequence. The audit-discipline skills still exist for non-hunter contexts but the hunters replace them here."
