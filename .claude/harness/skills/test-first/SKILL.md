...
2. **What's the classification?** One of:
   ...
   - **FLAKY** — reproducible 2/3 or worse with no clear code gap. Log but don't fix without more info. **Flakiness budget**: if the same reproducer or test fails flakily **3 times** within a rolling 14-day window without a real underlying bug surfacing, rewrite it. A flaky reproducer is worse than no reproducer — it erodes trust in the regression layer and trains the team to ignore red exits. Prefer rewriting the test to make it deterministic over adding retries or timeouts. Track flake counts in the bug entry's `Classification:` line — `CONFIRMED (flake_count: 2)` makes the next agent decide at 3.
...
## Step 0.5 — Localize the bug before writing a reproducer

Before you write the reproducer (Step 1) or touch any code, **pinpoint exactly where the bug lives**. Fault-localization is where 80% of the intellectual work of a bug fix happens — the Agentless paper (ICLR 2025) proved this. Write the reproducer *against the localized site*, not against a hazy guess of where the problem is.

Three tiers of localization, always in this order:

### Tier 1 — File-level

Narrow from "somewhere in the repo" to 1–5 candidate files. Use whatever gives you the fastest signal:

- **If there's a stack trace:** the top frames name the files. Read them first.
- **If there's a failing test:** that test's imports + the SUT it exercises are your candidates.
- **If there's a reproduction script or UI flow:** trace which files handle the request/event path — grep for route handlers, component names, event names mentioned in the symptom.
- **If there's only a user-visible symptom:** grep for the exact strings/labels the user saw. Those strings live in the code that rendered or logged them.

Stop as soon as you can name specific files. "Candidate files: `src/lib/auth/session.ts` and `src/lib/auth/token-refresh.ts`" — not "it's probably somewhere in auth".

### Tier 2 — Function-level

Within each candidate file, extract the 1–3 functions most likely to be involved:

- Functions referenced in the stack trace (obvious).
- Functions whose names match the operation in the symptom ("session expired" → search for `expire`, `expiry`, `ttl`, `renew`, `refresh`).
- Functions recently modified — check `git log -5 --oneline -- <file>` or `git blame` for suspicious recent edits.

If a candidate file is small (< 200 lines), you can skip to Tier 3 directly.

### Tier 3 — Edit-site-level

Within each candidate function, pin the 2–5 line span most likely to need changing:

- A specific condition (`if (...)`), comparison, or constant that looks wrong given the symptom.
- A branch that's entered incorrectly, skipped incorrectly, or handles the wrong case.
- A missing check, a misplaced await, a timezone that should be UTC but isn't.

Write the edit site down before touching it: `src/lib/auth/session.ts:142` — `new Date(Date.now() + ttl * 1000)` assumes `ttl` is seconds, but the config supplies minutes.

### Why this step matters

- **Reproducer quality goes up**. A static-grep reproducer against a localized site is far cheaper and more durable than a live-behavioral reproducer guessing at the problem surface.
- **Blast-radius answer gets concrete** (Step 0 question 4). Without localization, "blast radius" is vague; with it, you know exactly which callers could be affected.
- **The "push the guard to the invariant" rule (Step 2) only works if you've found the invariant** — which is what Tier 3 gives you.

### Red flags

- Spending more than 30 minutes in Tier 1 without narrowing the candidate list. If grep + stack-trace reading isn't converging, re-read the symptom; you may be looking in the wrong subsystem.
- Jumping to Tier 3 without Tier 1/2. You'll pick a plausible-looking site that turns out not to be the root cause.
- Writing a reproducer at the symptom layer when the actual bug is two layers deeper. The reproducer should target the pinned edit-site, not the user-visible effect (unless the edit-site test would be more fragile).

### Fast-path carve-out

If Step 0 gives you CONFIRMED + a symptom that directly names a single code construct (e.g. "the literal string 'Session expired' appears" — grep it), localization can be one command. Don't ceremonially walk three tiers for a trivial localization. The tiers exist to catch *hard* bugs, not to slow down obvious ones.
...
4. **Append a one-line entry to `.claude/test-runs/reproducers/INDEX.md`.** The index is the scannable table of every reproducer in the tree — bug id, one-line symptom, file path, what layer it exercises (static grep / unit / live behavioral). Format:
   ```markdown
   | BUG-NNN | <one-line symptom> | `bug-NNN.sh` | static-grep | CONFIRMED |
   ```
   The index is read by `fix-loop` and `system-test-loop` to pick regression checks during sweeps, and by the research loop to cluster recurring failure modes. Don't ship a new reproducer without adding its index line — a reproducer not in the index is a reproducer future agents won't find.
