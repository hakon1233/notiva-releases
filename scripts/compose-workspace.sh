...
compose_root_file() {
  ...
  cat > "$out" <<EOF
<!--
  AUTO-GENERATED BY scripts/compose-workspace.sh — DO NOT EDIT THIS FILE.

  This file is the composed view of:
    .claude/harness/$layer_name.part.md  (harness layer — upstream, auto-updated)
    .claude/project/$layer_name.part.md  (project layer — your customizations)

  To change something from the harness: edit the template in the TTM repo and
  bump the harness version. Your change lands fleet-wide on the next deploy.

  To change something project-specific: edit .claude/project/$layer_name.part.md
  and run bash scripts/compose-workspace.sh.

  Precedence: when two instructions conflict, the project layer below wins.
-->
...
EOF
  ...
  # Always separate with a heading so the two layers are visible in the composed
  # file even if both parts end/start without blank lines.
  cat >> "$out" <<'SEPARATOR'


<!-- =============================================================
     PROJECT-SPECIFIC LAYER (editable in .claude/project/*.part.md)
     Instructions below override anything above on conflict.
     ============================================================= -->

SEPARATOR

  if [[ -f "$PROJECT/$layer_name.part.md" ]]; then
